...
$trips = null;  // Initialise la variable avant de l'utiliser

if (isset($_GET['id'])) {
    $trips_id = $_GET['id'];
    $stmt = $pdo->prepare("SELECT * FROM trips WHERE id = ?");
    $stmt->execute([$trips_id]);
    $trips = $stmt->fetch(PDO::FETCH_ASSOC);
}

// Vérifie si $trips contient des données avant de l'utiliser
if (!$trips) {
    echo "Aucune information trouvée pour ce voyage.";
...
<head>
    ...
    <title>Réservation - <?= htmlspecialchars($trips['title']); ?></title>
...
<body>
    ...
    <div class="container mt-5">
        <h2 class="text-center mb-4">Book for<span style="color: #1f2e4e;"> <?= htmlspecialchars($trips['title']); ?></span></h2>
        <div class="row">
            <div class="col-md-6 mx-auto">
                <div class="card shadow-sm">
                    <div class="card-body">
                        <h5 class="card-title"><?= htmlspecialchars($trips['title']); ?></h5>
                        <img src="uploads/<?= htmlspecialchars($trips['image']) ?>" alt="<?= htmlspecialchars($trips['title']) ?>" class="img-fluid w-100 mb-4" style="height: 200px; object-fit:cover; width:100%; border-radius:10px">
                        <p class="card-text"><?= htmlspecialchars($trips['description']); ?></p>
                        <p class="card-text"><strong>price:</strong> <?= number_format((float)$trips['price'], 2); ?> USD</p>
                        <label for="people">Number of people:</label>
                        <input type="number" name="people" id="people" class="form-control mb-3" min="1" max="<?= $trips['available_places']; ?>" required>
                        <p class="card-text"><strong>Availability:</strong> <?= htmlspecialchars($trips['available_places']); ?> Remaining Places</p>
    ...
    const tourTitre = <?= json_encode($trips['title']) ?>; // Titre du voyage

    // Vérification si les champs sont remplis
    if (!people || !preferredDate) {
        alert("Veuillez remplir tous les champs avant de soumettre le formulaire.");
        return; // Empêche la soumission du formulaire si un champ est vide
    }
